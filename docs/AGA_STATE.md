...
- A live frontier ledger now scores visible settlement prospects, surfaces the lead rebuild line in the atlas, and feeds the arrival dossier with viability, trust, pressure, and next-action guidance drawn from route intel and Brinehook aftermath.
...
### 0051 - Frontier Ledger Viability Surface

- **State Assessment:** The world already knew which routes were charted, which settlements looked promising, and how Tidewalk trust or Black-Keel pressure landed after Brinehook, but none of that rolled up into a readable rebuild signal. The single greatest bottleneck was decision clarity: the player could unlock prospects, yet could not tell which foothold was strengthening, collapsing, or worth pursuing next.
- **Strategic Choice:** B. Systemic Expansion.
- **Justification:** The right move was a compact systems layer, not another destination. A frontier ledger makes existing route, survey, and aftermath state legible enough to drive real settlement choices without widening the map or inventing a second simulation stack.
- **Plan Critique:** A larger biome or second arrival sequence before surfacing viability would have deepened content faster than comprehension. The corrected scope was to keep all new work read-only: add a pure selector, thread it into the atlas and arrival dossier, and prove the resulting lead-prospect logic under both Lantern Line and Black-Keel outcomes.
- **Execution Plan:**
  - **Specific Tasks:** Add route-level settlement potential data plus a pure frontier-ledger selector; score visible prospects from charting, traversal, encounter resolution, and Tidewalk trust/pressure; surface the lead prospect and per-route viability in the atlas and arrival dossier; add deterministic tests for neutral, pressured, and secured Tidewalk outcomes.
  - **Technology Stack Justification:** The current vanilla Canvas/DOM client and deterministic Node test harness remain the right stack because the missing work was systems readability, not engine capability. A pure selector module keeps the logic portable for later richer UI or packaging targets.
  - **Success Metrics:** `tests/frontier-ledger.test.mjs` passes; `tests/run-all.mjs` stays green; the served page exposes the new frontier-ledger strings in the live HTML; the atlas route list now shows stage/viability instead of only raw route state.
  - **Risk Mitigation:** Keep the ledger read-only and derived from existing state so there is no save-schema bump, no migration risk, and no hidden progression mutation. Route settlement potential stays authored data on existing routes, which is easy to rebalance later.
- **Work Completed:** Added `src/frontier-ledger.js`; authored route settlement-potential values plus route-summary support in `src/game-state.js`; surfaced lead-prospect, next-action, and per-route viability in the atlas and arrival dossier through `src/game.js`, `index.html`, and `src/styles.css`; added `tests/frontier-ledger.test.mjs` and registered it in `tests/run-all.mjs`; updated `README.md` and the current-state summary to describe the new rebuild-viability surface.
- **Validation Evidence:** `C:\Users\MichaelLaurenzo\.cache\codex-runtimes\codex-primary-runtime\dependencies\node\bin\node.exe --check src/frontier-ledger.js` passed. `C:\Users\MichaelLaurenzo\.cache\codex-runtimes\codex-primary-runtime\dependencies\node\bin\node.exe --check src/game.js` passed. `C:\Users\MichaelLaurenzo\.cache\codex-runtimes\codex-primary-runtime\dependencies\node\bin\node.exe --check src/game-state.js` passed. `C:\Users\MichaelLaurenzo\.cache\codex-runtimes\codex-primary-runtime\dependencies\node\bin\node.exe tests/frontier-ledger.test.mjs` passed. `C:\Users\MichaelLaurenzo\.cache\codex-runtimes\codex-primary-runtime\dependencies\node\bin\node.exe tests/run-all.mjs` passed all 31 suites. Local HTTP smoke on `http://127.0.0.1:5191/` returned `200` and confirmed `Signal Below`, `Frontier Ledger`, and the arrival-ledger markup in served content. Browser-connector smoke was attempted but unavailable because the Opera connector was not connected, and the in-app browser control bridge reset during long local-page calls.
- **External Services Used:** GitHub remained the repository remote. Browser validation was attempted through available browser surfaces, but no external browser project or service was modified.
- **Learned Constraints:** The local app can still be smoke-checked reliably with the static server plus HTML assertions even when a browser connector is not currently linked. The frontier ledger should stay derived from world state until there is a second playable foothold; persisting separate ledger state now would only duplicate truth.
- **Next Bottleneck:** Turn one rival ledger prospect beyond Tidewalk, preferably Fenward Ranger Enclave or Stoneward Bastion, into a real playable arrival/foothold so the new settlement-viability readout points at more than one tangible expansion path.
