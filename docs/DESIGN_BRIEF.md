...
The first settlement foothold is Tidewalk Coast. Once the archive lift route is linked, the arrival dossier now exposes a state-backed edge encounter where the player secures docking rights with Tidelantern Quay. That handoff now continues into a short on-foot Tidewalk survey descent: the player returns to the Coastline Lift, enters the drowned warehouses physically, stabilizes survey rings under black-tide pressure, and only then brings a hostile salvage mark back inland. The result is a tighter wilderness-survey pattern that can scale into future biome scouting and faction trail work.
